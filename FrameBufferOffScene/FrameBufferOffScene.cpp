...
#pragma endregion
	...
	Shader offsceneShader("OffScene.vertex","OffScene.frag");
	Shader sceneShader("Scene.vertex", "Scene.frag");
	offsceneShader.use();
	offsceneShader.setInt("texture_diffuse1", 0);
	offsceneShader.unUse();
	
	sceneShader.use();
	sceneShader.setInt("texture_diffuse1", 0);
	sceneShader.unUse();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		offsceneShader.setMat4("view", view);
		offsceneShader.setMat4("projection", projection);
		// cubes
		...
		offsceneShader.setMat4("model", model);
		...
		glClearColor(0.18f, 0.04f, 0.14f, 1.0f); // set clear color to white (not really necessary actually, since we won't be able to see behind the quad anyways)
		glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);

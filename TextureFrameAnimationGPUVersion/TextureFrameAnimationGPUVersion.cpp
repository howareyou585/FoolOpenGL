...
int main(int argc, char** argv)
{
	...
	// Section2 准备着色器程序
	Shader shader("TextureFrameAnimationGPUVer.vertex", "TextureFrameAnimationGPUVer.frag");
	shader.use();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float scale = 1.0f / row;
		glm::vec4 frameInfo(curRow, curCol,frame, scale);
		...
		shader.use();
		shader.setVec4("framInfo", frameInfo);

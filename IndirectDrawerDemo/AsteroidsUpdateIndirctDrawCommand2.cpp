...
int main(int argc, char** argv)
{
	...
	for (unsigned int i = 0; i < amount; i++)
	{
		...
		model = glm::scale(model, glm::vec3(scale*10));
	...
	vector<DrawElementsIndirectCommand> vecDrawElementsIndirectCommand;
	vecDrawElementsIndirectCommand.reserve(amount);
	for (int i = 0; i < amount; i++)
	{
		...
		vecDrawElementsIndirectCommand.emplace_back(ptrIndirectCommands[i]);
	}
	keyValue = 3;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 这里填写场景绘制代码
		rockShader.use();
		//keyValue = ((unsigned long)glfwGetTime()) % amount;
		rockShader.setVec4("uColor", vecColor[keyValue]);
		int error = glGetError();
		if (error != 0)
		{
		...
		if (dirty)
		{
			...
			memcpy(ptrIndirectCommands, &vecDrawElementsIndirectCommand[keyValue], sizeof(DrawElementsIndirectCommand));
		}

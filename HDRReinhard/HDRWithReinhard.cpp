...
int main(int argc, char** argv)
{
	...
	VAOBuffer squareVaoBuffer;
	
	vecAttrib.emplace_back(vertex_attribute::position);
	vecAttrib.emplace_back(vertex_attribute::texcoord);
	mapAttrib2Size[vertex_attribute::position] = 3;
	mapAttrib2Size[vertex_attribute::texcoord] = 2;
	squareVaoBuffer.BuildVAO(squareVertices, sizeof(squareVertices), squareIndexes,
		sizeof(squareIndexes), vecAttrib, mapAttrib2Size);
	GLuint squareVAOId = squareVaoBuffer.GetVAO();
	GLuint squareVBOId = squareVaoBuffer.GetVBO();
	GLuint squareEBOId = squareVaoBuffer.GetEBO();
	//加载材质
   ...
   // - Positions
	...
	Shader hdrShader("hdrReinhard.vertex", "hdrReinhard.frag");
	Shader lightCube("lightCube.vertex", "lightCube.frag");
	shader.use();
	...
	lightCube.use();
	lightCube.setMat4("projection", projectionMatrix);
	lightCube.unUse();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < lightPositions.size(); i++)
		{
			...
			lightMatrix = glm::translate(lightMatrix, lightPositions[i]);
			lightMatrix = glm::scale(lightMatrix, glm::vec3(0.2, 0.2, 0.2));
			lightCube.setMat4("model", lightMatrix);
			lightCube.setMat4("view", viewMatrix);
			lightCube.setVec3("ucolor", lightColors[i]);
			glDrawArrays(GL_TRIANGLES, 0, nVertex);
		}
		lightCube.unUse();
